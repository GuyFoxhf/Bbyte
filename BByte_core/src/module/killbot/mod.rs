...
impl Session for ServerKill {
    ...
    fn write(&mut self, data: &Vec<u8>) -> std::io::Result<()> {
        if data.len() == 3 && self.alive() && data == &vec![MAGIC_FLAG[0], MAGIC_FLAG[1], 0xff] {
            log::info!("client closed kill bot");
            self.close();
            return Ok(());
        }
        // return Ok(());
