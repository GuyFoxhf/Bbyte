...
pub unsafe fn NtGetTickCount64() -> ULONGLONG {
    ...
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))] {
        let tick_count_quad = unsafe { (*USER_SHARED_DATA).u.TickCountQuad };
        let tick_count = tick_count_quad; 
        // let tick_count = unsafe { read_unaligned(tick_count_ptr) };
    }
    #[cfg(target_arch = "x86")] {
        loop {
...
pub unsafe fn NtGetTickCount() -> ULONG {
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
    {
        ...
        let tick_count_quad = (*USER_SHARED_DATA).u.TickCountQuad;
        let tick_count = tick_count_quad * (*USER_SHARED_DATA).TickCountMultiplier as u64;
       
        (tick_count >> 24) as u32
    }
    #[cfg(target_arch = "x86")] {
        let mut tick_count: ULARGE_INTEGER = uninitialized();
        loop {
