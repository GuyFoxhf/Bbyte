...
impl App for FtpApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.selectable_value(&mut self.switch, SwitchDock::Transfer, "Payload Inject");
...
impl FtpApp {
    fn render_combobox_with_label(&mut self, ui: &mut egui::Ui, x: f32, y: f32) {
        ...
        ui.horizontal(|ui| {
            ...
            let label_pos = egui::Pos2::new(x-153.0, y+12.0);  
            ui.painter().text(
                label_pos,
                egui::Align2::LEFT_TOP,  
                "Select inject process",  
                egui::TextStyle::Heading.resolve(&ui.style()), 
                ui.style().visuals.text_color(),  
            );
 
            let combo_width = 200.0;  
            let combo_height = 30.0;  
            let combo_rect = egui::Rect::from_min_size(
                egui::Pos2::new(x + 10.0, y),  
                egui::vec2(combo_width, combo_height),  
            );
    
 
            let frame = egui::Frame {
                inner_margin: egui::style::Margin::default(),
                ..Default::default()
            };
    
            frame.show(ui, |ui| {
                // Позиционируем ComboBox
                ui.allocate_space(egui::vec2(x, y));
                egui::ComboBox::from_id_source("program_selector")
                    .selected_text(&self.selected_program)
                    .width(280.0)
                    .show_ui(ui, |ui| {
 
                        ui.selectable_value(&mut self.selected_program, "calc.exe".to_string(), "calc.exe");
                        ui.selectable_value(&mut self.selected_program, "notepad.exe".to_string(), "notepad.exe");
                        ui.selectable_value(&mut self.selected_program, "cmd.exe".to_string(), "cmd.exe");
                        ui.selectable_value(&mut self.selected_program, "taskmgr.exe".to_string(), "taskmgr.exe");
                        ui.selectable_value(&mut self.selected_program, "msconfig.exe".to_string(), "msconfig.exe");
                        ui.selectable_value(&mut self.selected_program, "dxdiag.exe".to_string(), "dxdiag.exe");
                        ui.selectable_value(&mut self.selected_program, "systeminfo.exe".to_string(), "systeminfo.exe");
                        ui.selectable_value(&mut self.selected_program, "eventvwr.exe".to_string(), "eventvwr.exe");
                        ui.selectable_value(&mut self.selected_program, "mmc.exe".to_string(), "mmc.exe");
                        ui.selectable_value(&mut self.selected_program, "perfmon.exe".to_string(), "perfmon.exe");
                        ui.selectable_value(&mut self.selected_program, "control.exe".to_string(), "control.exe");
                        ui.selectable_value(&mut self.selected_program, "write.exe".to_string(), "write.exe");
                        ui.selectable_value(&mut self.selected_program, "mstsc.exe".to_string(), "mstsc.exe");
                        ui.selectable_value(&mut self.selected_program, "cleanmgr.exe".to_string(), "cleanmgr.exe");
                        ui.selectable_value(&mut self.selected_program, "rundll32.exe".to_string(), "rundll32.exe");
                        ui.selectable_value(&mut self.selected_program, "msiexec.exe".to_string(), "msiexec.exe");
                        ui.selectable_value(&mut self.selected_program, "wscript.exe".to_string(), "wscript.exe");
                        ui.selectable_value(&mut self.selected_program, "cscript.exe".to_string(), "cscript.exe");
                        ui.selectable_value(&mut self.selected_program, "netstat.exe".to_string(), "netstat.exe");
                        ui.selectable_value(&mut self.selected_program, "ipconfig.exe".to_string(), "ipconfig.exe");
                        ui.selectable_value(&mut self.selected_program, "ping.exe".to_string(), "ping.exe");
                        ui.selectable_value(&mut self.selected_program, "tracert.exe".to_string(), "tracert.exe");
                        ui.selectable_value(&mut self.selected_program, "route.exe".to_string(), "route.exe");
                        ui.selectable_value(&mut self.selected_program, "nslookup.exe".to_string(), "nslookup.exe");
                        ui.selectable_value(&mut self.selected_program, "net.exe".to_string(), "net.exe");
                        ui.selectable_value(&mut self.selected_program, "whoami.exe".to_string(), "whoami.exe");
                        ui.selectable_value(&mut self.selected_program, "sc.exe".to_string(), "sc.exe");
                        ui.selectable_value(&mut self.selected_program, "schtasks.exe".to_string(), "schtasks.exe");
                        ui.selectable_value(&mut self.selected_program, "tasklist.exe".to_string(), "tasklist.exe");
                        ui.selectable_value(&mut self.selected_program, "taskkill.exe".to_string(), "taskkill.exe");
                         ui.selectable_value(&mut self.selected_program, "wevtutil.exe".to_string(), "wevtutil.exe");
                        ui.selectable_value(&mut self.selected_program, "logoff.exe".to_string(), "logoff.exe");
                        ui.selectable_value(&mut self.selected_program, "shutdown.exe".to_string(), "shutdown.exe");
                        ui.selectable_value(&mut self.selected_program, "diskpart.exe".to_string(), "diskpart.exe");
                        ui.selectable_value(&mut self.selected_program, "bcdedit.exe".to_string(), "bcdedit.exe");
                        ui.selectable_value(&mut self.selected_program, "fsutil.exe".to_string(), "fsutil.exe");
                        ui.selectable_value(&mut self.selected_program, "chkdsk.exe".to_string(), "chkdsk.exe");
                        ui.selectable_value(&mut self.selected_program, "sfc.exe".to_string(), "sfc.exe");
                        ui.selectable_value(&mut self.selected_program, "dism.exe".to_string(), "dism.exe");
                    });
            });
...
fn main() {
    ...
    eframe::run_native(
        &format!("Bbyte Inject Beta - {}", title),
        options,
